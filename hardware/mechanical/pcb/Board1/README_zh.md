...
舵机(板级网表历史): H4-D6/D7 → Y舵机-3 / X舵机-3
  ※ 当前固件改走 D3/D9（见下文「与固件差异」），若用本网表 PCB 需飞线或改版
...
丝印与 GPIO 对应见 `firmware/deskbot_config.h`（**以固件为准**）：
...
- D4=GPIO5 (I2S LRC)
- D5=GPIO6 (I2S BCLK)
- D6=GPIO43 / D7=GPIO44（UART0；**勿再作舵机**）
- D8=GPIO7 (LCD SCK)
- D9=GPIO8 (**X 舵机**，当前固件)
- D10=GPIO9 (LCD MOSI)
...
早期 Board1 网表把舵机接到 **H4-D6/D7**。当前固件为避开 UART0，改为：
...
H3 已含 **D3**；H4 含 **D9**。若手里的转接板仍把舵机座焊在 D6/D7 网络上，需飞线到 D3/D9，或改 PCB 走线后再生产。

## 注意事项

1. **SPK+/SPK- 引脚顺序**：本重建依据说明书「头部喇叭线 → 转接板 → 功放板」走线推断；若你手上有已装好的板子，用万用表通断档核对 vcc-7/8 与 vin-6/7 最稳妥。
2. **D3 / D9** 在插座上均有引出（H3-D3、H4-D9）；当前固件用作舵机 PWM，**不要**再标成 NC。
