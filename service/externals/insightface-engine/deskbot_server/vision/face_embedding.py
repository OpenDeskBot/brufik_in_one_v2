...
# 来源: src/deskbot_server/vision/face_embedding.py
# 同步: 主服务改动后按 docs/external_services.md「insightface-engine」节同步本副本并更新日期
# synced: 2026-08-30
# 分叉点: _resolve_recognition_onnx 优先本地 models/buffalo_s/（见函数内标记）
...
# === 分叉点 ===
# 与源文件差异：_resolve_recognition_onnx 优先本地 models/buffalo_s/（install.sh 落地，完全自包含），
# 再走 env 覆盖，最后才回退 insightface 自动下载（~/.insightface 共享缓存）。
# 其余逻辑与源文件一致，重新同步时手工重放本段。
def _resolve_recognition_onnx(pack: str) -> str:
    ...
    # 【分叉点】本地模型副本优先（运行期零依赖主服务；install.sh 从主服务 models/ 或
    # ~/.insightface 缓存 copy，无源则下载 GitHub release）
    from deskbot_server.utils.paths import MODELS_DIR

    for path in sorted(glob.glob(str(MODELS_DIR / pack / "*.onnx"))):
