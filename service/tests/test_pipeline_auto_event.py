...
def test_publish_auto_dispatch_event():
    ...
    async def _run() -> None:
        ...
        await bus.publish_auto_dispatch("dev1", request_id="req_chat_1", source="asr", summary="用户说了 hello")
        await bus.publish_auto_dispatch("dev1", request_id="req_auto_3", source="auto_idle_silence", summary="idle 再次")
        items = bus.snapshot("dev1")
        assert len(items) == 2
